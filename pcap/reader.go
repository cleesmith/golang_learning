...
type recordReader struct {
	...
	nb uint32    // number of unread bytes for current file entry
}
...
// numBytes returns the number of bytes left to read in the current file's enpry
...
// Read reads from the current enpry in the tar archive.
// It returns 0, io.EOF when it reaches the end of that enpry,
// until Next is called to advance to the next enpry.
